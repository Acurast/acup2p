...
impl NodeInner {
    ...
    pub(crate) fn listen_on_relay(&mut self, peer_id: &PeerId) -> Result<(), Error> {
            ...
            .listen_on(relay.clone().with(Protocol::P2pCircuit))
        {
            ...
            tracing::debug!(relay=%addr, error=%e, "failed to establish circuit relay");
            return Err(Error::NoRelayListener(addr.clone()));
        };
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::NoRelayListener(addr) => {
                write!(f, "Failed to establish circut relay at address {addr})")
            }
        }
    }
}
